...
@Controller('razorpay-config')
export class RazorpayConfigController {
  private readonly logger = new Logger(RazorpayConfigController.name);
  ...
  async getRazorpayConfig() {
    this.logger.log('RazorpayConfigController: getRazorpayConfig method called');
    return {
      message: 'Razorpay configuration from dedicated controller',
  ...
  async getPaymentSettings() {
    this.logger.log('RazorpayConfigController: getPaymentSettings method called');
    return {
      message: 'Payment settings from dedicated controller',
      keyId: this.configService.get<string>('RAZORPAY_KEY_ID'),
